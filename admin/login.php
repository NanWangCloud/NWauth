...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        $login_res = adminLogin($username, $password);
        if ($login_res['status']) {
            redirect(get_setting('admin_path', 'admin') . '/admin.php');
        } else {
            $error = $login_res['msg'];
            if ($error === '密码错误') {
                $error .= '。<a href="emergency_reset.php" class="alert-link">点击此处重置管理员密码</a>';
            }
        }
    }
}
...
<head>
    ...
    <title>管理后台登录 - <?php echo get_setting('site_name'); ?></title>
    <!-- 高速 CDN 镜像 -->
    ...
    <style>
        ...
        .login-card { width: 100%; max-width: 400px; padding: 2.5rem; border: none; border-radius: 20px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.05); background: white; margin: 15px; }
        ...
        .form-control { padding: 0.8rem 1.2rem; border-radius: 12px; border: 1px solid #e2e8f0; background-color: #fbfcfe; }
...
<body>
    <div class="login-card">
        <div class="text-center">
            ...
            <h4 class="fw-bold mb-1">管理后台</h4>
            <p class="text-muted small mb-4">请输入管理员凭据登录</p>
        </div>
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                <label class="form-label small fw-bold text-muted">管理员账号</label>
            ...
            <button type="submit" class="btn btn-primary w-100 mb-3">进入管理面板</button>
